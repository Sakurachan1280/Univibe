...
import { getSongsByGenre } from "../../API/songAPI";
...
type GenrePlaylistRouteProp = RouteProp<RootStackParamList, "GenrePlaylist">;
...
// Gradient palette per genre
const GENRE_CONFIG: Record<string, { colors: [string, string, string, string]; icon: string; label: string }> = {
    pop: {
        colors: ["#EC4899", "#9333EA", "#1a002a", "#050505"],
        icon: "musical-notes",
        label: "Pop",
    },
    ballad: {
        colors: ["#3B82F6", "#1D4ED8", "#0a001a", "#050505"],
        icon: "heart",
        label: "Ballad",
    },
    rap: {
        colors: ["#F59E0B", "#D97706", "#1a0a00", "#050505"],
        icon: "mic",
        label: "Rap",
    },
    rock: {
...
const DEFAULT_CONFIG = {
    colors: ["#6D28D9", "#4C1D95", "#0d001a", "#050505"] as [string, string, string, string],
    icon: "musical-note",
    label: "Nhạc",
};
...
export default function GenrePlaylistScreen() {
    ...
    const route = useRoute<GenrePlaylistRouteProp>();
    const { genre, title } = route.params;
    ...
    const config = GENRE_CONFIG[genre.toLowerCase()] ?? DEFAULT_CONFIG;
            ...
            >
                    ...
                    <SafeAreaView edges={["top"]}>
                        ...
                        <View style={{ paddingHorizontal: 24, alignItems: "center" }}>
                            ...
                            <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                                <Ionicons name="musical-notes" size={13} color={config.colors[0]} />
                                <Text style={{ color: "rgba(255,255,255,0.55)", fontSize: 13 }}>
                                    {isLoading ? "Đang tải..." : `${songs.length} bài hát${totalMin > 0 ? ` • ${totalMin} phút` : ""}`}
                                </Text>
                            </View>
                        </View>
                    </SafeAreaView>
                ...
                {isLoading ? (
                    ...
                    <View style={{ paddingTop: 60, alignItems: "center", paddingHorizontal: 32 }}>
                        ...
                        <TouchableOpacity onPress={loadSongs} style={{ paddingHorizontal: 24, paddingVertical: 12, borderRadius: 24, backgroundColor: config.colors[0] }}>
